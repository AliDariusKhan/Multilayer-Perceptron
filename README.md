{
 "cells": [
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "We will generate $a_i^{l+1}$ according to equation (1), but the first element will be overwritten such that $a_0^{l+1} = 1$. Therefore, the first row of the weight matrices, $\\Theta^l_{0j}$, does not represent real connections except with $l=L=1$ (because the final layer has no bias node)."
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "For the same reason, when we backpropagate the error according to equation (5), we will delete the first element of $z^l$ (where $l\\neq L$) and the first row $\\Theta^l_{0j}$, so that we do not backpropagate the errors through false connections. "
   ]
  },
  {
   "cell_type": "code",
